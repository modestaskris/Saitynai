<template>
  ...
  <button
    @click="onModalClick"
    class="block text-white bg-gray-800 hover:bg-black focus:ring-4 focus:outline-none focus:ring-gray-500 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
    type="button"
  ...
  <div
    ...
    :class="{ hidden: displayModal, shake: !displayModal }"
  ...
  >
    <div class="relative p-1 w-full flex justify-center h-fit">
      <!-- Modal content -->
      <div
        @click="preventBubbles"
        class="relative bg-white rounded-lg shadow dark:bg-gray-700 w-2/4"
      >
        <button
          @click="onModalClick"
          type="button"
  ...
  methods: {
    ...
    async onAddCategory() {
      if (this.isCategory) {
        ...
        this.$emit("create-button-pressed", this.newModelName);
      } else if (this.isPlaylist) {
        if (this.isPlaylistEmpty) {
          throw Error("Input fields is empty");
        }
        this.$emit("create-button-pressed", this.newModelName, this.newUrl);
      } else if (this.isSong) {
        if (this.isSongEmpty) {
          throw Error("Input fields is empty");
        }
        // NewModelName is
        this.$emit("create-button-pressed", "unusedField", this.newUrl);
      }
      this.clearInput();
      this.onModalClick();
    },
